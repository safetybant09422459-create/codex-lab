...
50ケースのmock baselineはClarification & Candidate Fallback v0.1適用前が45 pass、5 fail、適用後が48 pass、2 failである。`query_too_broad`は3件から0件になった。全件passを目標にせず、残る曖昧解決と文脈follow-upの失敗も期待値として保持する。固定proposalを使うため、このbaselineが測るのは主にOrchestrator、Search Index、Context適用、Clarification Policyであり、LLM Planner自体の品質はliveモードで別途確認する。
