...
Benchmark Versionは`Jarvis Benchmark v0.1`である。レポートへ常に記録し、将来の評価仕様変更後も同一version間で比較できるようにする。
...
8. Skill別のLayer SummaryとTop Improvement Targetsを含むJSON／Markdown reportを出力する。
...
全ケースは`skill_id`を持つ。未指定時は現在の既定値`travel`を使う。JSONの`layer_summary`は`skill_id -> failure_layer -> count`、`top_improvements`は件数降順のSkill／Layerリストである。MarkdownもSkillごとにLayer Summaryを分ける。このためPhoto、Garden、Calendar、Memoryのケースは、同じHarnessへ`skill_id`付きで追加できる。
...
50ケースのmock baselineは全件passを目標にしていない。自然な地域表現、memoの言い換え、年月・期間、広い指示語、文脈follow-upを期待値として保持し、改善時にfailがpassへ変わることを追跡する。固定proposalを使うため、このbaselineが測るのは主にOrchestrator、Search Index、Context適用であり、LLM Planner自体の品質はliveモードで別途確認する。

Markdown末尾のTop Improvement TargetsはSkill／Layerごとの件数順で生成される。たとえば`entity_resolution`はSearchDocumentの地域情報・別名・識別属性、`context`はConversationStateの解決済みEntity保持を改善候補として示す。これは優先順位決定用の診断であり、Benchmark実行自体は検索・Planner・Chat Coreを変更しない。
