...
この文書は、Travel DB実装前に、Travel Runtime / Executor / Repository / Storage の責務境界を整理するための設計メモである。

今回は設計のみを扱う。DB作成、Migration、Runtime Tool追加、Executor追加、API追加、UI追加、Immich連携、Google Places連携、Service Restartは行わない。

Travelの正は、今後作る新Travel DBである。既存の`/mnt/nas/projects/project/travel.db`はLegacy Dataであり、移行元、参照元、サンプルとして扱う。TravelExecutorやUIがLegacy DBへ直接依存する構造にはしない。
...
| --- | --- | --- | --- |
| Runtime | Tool呼び出し、権限、確認、監査、実行制御 | ExecutorのTool境界 | DBスキーマ詳細 |
...
| name | 用途 | 主な責務 |
| --- | --- | --- |
| `SQLiteTravelStorage` | 新Travel DBの本命Storage | 新Travel Schemaに基づく読み書き |
...
`TravelStorage`は新Travel DBの読み書きを担う名前として使う。`TravelSource`はLegacy、InMemory、Remoteなど、必ずしも新Travel DBを正としない読み取り元にも使える名前として扱う。
...
TravelRepository
  ↓
LegacyTravelSource or SQLiteTravelStorage
```

---

## 6. 最初に実装するread-only Tool候補

最初はread-only Toolから始める。
...
最小のTool応答は、Trip一覧とTripごとのTimelineを返せればよい。Experience Card再現に必要な`display_title`、`place_name`、`start_at`、`end_at`、`cover_image`、`memo`、`participants`、`linked_photos`はRepositoryの返却モデルに含められるようにする。

---

## 7. write系Toolを後回しにする理由

write系Toolは後回しにする。

理由:

* 新Travel DBがまだ作られていない
* Legacy DBは将来的に廃止予定であり、書き込み先にすべきではない
* Trip、Timeline、Photo Link、Participant、Cover Imageは家族の予定、位置情報、写真、思い出に関わる
* 更新系はPermission / Confirmation / Auditが必要である
* AI自動編集を許可する前に、提案、確認、実行、取り消しの流れを決める必要がある
* まずread-onlyでRepository契約を固める方が、UI / MCP / Chatの共通利用を検証しやすい

将来write系を追加する場合も、RuntimeがPermission / Confirmation / Auditを担当し、ExecutorはRepositoryへ確定済み操作を渡す形にする。
...
| --- | --- | --- |
| Legacy DB参照から新SQLite DBへ移行 | `LegacyTravelSource`から`SQLiteTravelStorage`への切り替え | Executor、Tool schema、UI |
...
6. 読み取り系か更新系か
   * 今回の設計と初期Tool候補は読み取り系である。更新系は新Travel DB、権限、確認、監査の方針が固まってから追加する。
