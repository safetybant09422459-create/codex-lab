...
この文書は次フェーズの設計メモであり、Response IntelligenceとTravel Answer Generatorは
未実装である。v0.1では既存APIとRuntime境界を保ったまま、Travelの読み取り質問を一つだけ
縦に通す。
...
LLM呼び出しはPlanner 1回とAnswer Generator 1回の合計最大2回に制限する。Toolは、名前付き
質問では`get_trips → get_trip_timeline`、選択中旅行では必要に応じて
`get_trip → get_trip_timeline`の最大2回を想定する。

名前付き質問について、現実装は`get_trips`で旅行を解決した後に`get_trip`で終了する。
v0.1実装ではPlanに「旅行を開く」ではなく「行動要約」「食事抽出」「日別要約」という回答目的と
必要Evidenceを保持し、旅行解決後に`get_trip_timeline`へ進める必要がある。
