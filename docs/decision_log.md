...
失敗、ユーザー訂正、選択ミス、Runtime失敗等を、ユーザー理解を汚染せずJarvis改善へどう残すか？

### 決定

失敗・訂正・改善シグナルはLong-term Contextへ混ぜず、Jarvis改善専用のLearning Logとして分離する。
Learning Logは通常回答の根拠やユーザーの好み・性格・方針に使わず、改善レビュー時だけLLMが参照する。

LLMはProvider Responsibility、Capability / Operation説明、Context Assembly、Long-term Context retrieval、
新Provider候補を提案できる。Pythonは保存、安全な分離、policy適用、決定的構造化、Runtime記録に限定する。
Provider、DB、API、Tool、コードの生成・追加・変更は自動実行せず、人間判断を必須とする。
