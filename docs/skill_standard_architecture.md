...
この文書は、JarvisのSkillと、そのCore向け能力境界であるDomain Providerを作るときの標準レイヤー構造を定義する。
...
Skillはユーザーから見える能力・機能単位、Domain ProviderはCoreから見える能力提供境界である。通常は
1つのSkillが同名ドメインのProvider契約を所有するが、両者は同義ではない。Providerは実装方式ではなく責務であり、
Local Service、REST API、MCPのどれで公開しても同じ契約を保つ。
...
RepositoryはProvider内部の正本境界である。UI、Runtime Executor、MCP Handler、Chat / AI Automationは
Repositoryへ直接結合せず、同じProvider Operation契約を利用する。

## Provider Contract

Providerはドメイン固有のOperationを、明確なschema、read/write、risk、error、source情報とともに公開する。
CoreはProviderのtransport、DB、外部API、Repository実装を知らない。

ProviderはCRUD、Repository / DBアクセス、検索、正本データ取得、外部API呼び出し、Operation実行、
ドメイン不変条件や正規化を担当する。内部の低レベルdispatch、Repository選択、外部API fallbackは許容する。

発話の意味理解、Provider / Operation選択、複数Providerの組み合わせ、Clarification、Persona、会話、
最終回答は担当しない。またRuntimeのValidation、Permission、Confirmation、Auditを代替しない。
write OperationはtransportにかかわらずRuntimeの安全境界を通す。
...
例えばTravelが写真候補を必要とする場合、TravelはImmich APIやImmich Adapterを直接呼ばない。Travelは
Photo SkillのProvider Operation、Tool、またはAPI契約を経由して、写真候補やThumbnail参照を受け取る。

```text
Travel Provider
↓
Photo Provider Operation / Tool / API
↓
PhotoRepository
↓
ImmichAdapter
...
3. 将来MCP Tool化できるか
   * できる。同じProvider ContractをMCP Handlerへ載せられる。
