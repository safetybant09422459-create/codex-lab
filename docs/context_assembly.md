...
5. **Memory RAG candidates**: 毎Turn薄く検索した上位3〜5件
...
## Memory RAG on every turn

MemoryはCapabilityであると同時にCore基盤である。LLMが「Memoryが必要」と気付くことだけに依存せず、
現在発話とWorking Contextをqueryとして毎Turn薄く検索する。

* 検索前に認証主体、owner、visibility、利用目的でfilterする
* 上位3〜5件、短いsummary、source、confidence、更新時刻、Evidence参照だけを返す
* Promptでは「関連する可能性があるMemory候補。事実として無条件に断定しない」と明記する
* 低score、矛盾、失効、Forget済み候補は渡さない
* 子ども、健康、位置、写真、予定などはより厳しい利用Policyを適用する
* 追加の探索が必要ならLLMはMemory Capabilityの詳細Toolを要求する

RAG検索はreadであるが、閲覧権限と外部Providerへの送信最小化が必要である。Memoryの作成、修正、
共有、Forgetは別のguarded writeであり、自動実行しない。
...
現在発話、認可済みUser Profile、Working Context、関連Memory、Capability概要、補助Current Worldの
...
現在の`POST /api/chat`は最大5TurnのWorking Contextを使い、Router / Turn decisionでBasic Chatと
Travelを分離する。Travel選択時はTravel Planner、Runtime、縮小EvidenceBundle、Final Answer LLMへ
進む最大3 LLM構成である。Current World、Capability Catalog、Memory RAG、完全な共通Turn契約は
未実装である。

本書は契約方針のみを定め、DB、Memory検索、Catalog generator、LLM呼び出し、API変更は実装しない。
