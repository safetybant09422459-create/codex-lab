...
Entity Context v0では、Domain Providerがread結果から宣言した正本ID付き候補をObservationへ格納し、
`EntityContextBuilder`が宣言順を変えずに直近20件まで`active_entities`へ保存する。Travelの最小実装は
`get_trips`結果の`id`と`title`から`trip`候補を作り、`source_provider`、`source_operation`、
`observed_at`、`visibility`を付ける。Context Builderの既存redactionとvisibility filterを通して次Turnの
LLMへ渡す。

これは参照解決やEntity確定ではない。Pythonは発話、label、過去topicを照合せず、候補のranking、Provider /
Operation選択、「それ」が指すIDの決定を行わない。永続Memory、write operation、Repository更新も含まない。

現時点のローカル個人利用Web Chatは、server-ownedな`family` principalとして扱う。これにより`family`
visibilityのEntity候補を既存filter通過後のContextへ含めるが、visibility filter自体は維持する。requestから
roleを受け取らず、本格的なユーザー認証とprincipal割当は将来対応とする。Entity Contextは候補を渡すだけで、
canonical IDを使う参照解決はLLMが行う。
