...
> 正本Entityの軽量な候補想起とTravel Provider PoCは
> [Jarvis Core Activation RAG](activation_rag.md)を参照する。Activation候補は未検証hintであり、
> Runtime / Evidence経路を迂回しない。
...
Capability Usage RAGはCatalog自体やTool schemaを検索するものではなく、「どのような依頼で、どの
Capabilityをどう使うか」という承認済み利用例を想起する将来の補助である。「神戸で何した？」は
Travel read、「写真ある？」はPhoto read、「テレビつけて」はHome Actionへ分類できるが、候補は
Tool callや実行許可ではない。特にAction ToolとMemory / Domain Searchを同じ実行経路とみなさない。
...
Receive User Message
  -> Context Assembly
       ...
       Activation RAG candidates / Capability Catalog / Current World / User Message
  -> LLM Turn
...
Activation RAGが所有するもの:

* 認可scope内のProvider横断候補検索とranking
* 再生成可能な`RagDocument`と未検証`RagSearchResult`の共通契約
* 検索障害をChatから分離するread-only境界

Activation RAGはDB、Repository、Runtime、Entity Resolver、Evidence Assemblyを所有しない。SQLiteと
Domain Repositoryだけを正本とし、Activation結果からEntityを確定またはToolを直接実行しない。
...
* Skill-neutralなFinal Answer LLMとTravel固有表示支援

既存のTravel Planner、Goal-aware Planning、Final Answer Generator、Travel Plan Executor、Travel
...
* **TravelのChat Core占有: v0.1で入口を分離済み。** `POST /api/chat`は`handle_chat()`を呼び、
  ...
  Executor、Final Answer Generator、Composerは既存Adapterとして維持している。現在のSkill会話は
  Router / Turn decision、Travel Planner、Final Answerの最大3 LLM構成である。
...
* **Pythonが自然言語理解を持ちすぎている: reduced / medium.** `TravelResponseComposer`は通常経路で
  `ClarificationPolicy`を呼ばず、候補の件数・形・上限など決定的な整形だけを担う。候補への
  聞き返しも取得済みEvidenceからFinal Answer LLMが生成する。`travel_plan_executor.py`のlegacy互換
  判定は残る。Travel専用の決定的回答fallbackは削除し、Final Answer LLM失敗時は意味解釈を行わない
  静的な取得結果へ退避する。
...
* **Memory RAGがない: confirmed / high.** Travelを最初のProviderとするActivation RAG PoCはあるが、
  Memory Provider、ownership / purpose policy、過去の訂正や好みの想起は未実装である。
...
4. Travel配線をTravel Skill Adapterへ包み、選択後だけ既存Planner / Executorを呼ぶ。既存API互換は
   Facadeで維持する。
5. [Memory Principle](decisions/2026-07-memory-principle.md)に従うread-only Memory Context契約を設計する。
   Pythonによる意味ranking、要約、重要度判定、自動想起は前提にしない。
...
4. Clarification / Response Composer
   - 曖昧性と回答目的はRouter / Planner LLMが決め、候補はResolverが返した既存候補だけを使う。
   - 候補Evidenceからの聞き返し文はFinal Answer LLMが生成する。Composerは候補数、候補shape、
     legacy契約を決定的に整形し、LLM失敗時だけ固定文へfallbackする。
...
`recommended_action`を持つ。`ClarificationPolicy`は互換テスト用に残すが、Composerの通常経路では
使用しない。「初めて」「最近」「どれか」などの意味判定や自然文生成はPlanner / Final Answer LLMへ
寄せる。
...
同一tabの`sessionStorage`へ最大5件だけ保持し、次のリクエストへ渡す。再読み込み後も同じtabでは復元するが、
別tab、別端末、長期Memoryへは引き継がない。`POST /api/chat/session/reset`は同じsession IDのserver-side bounded
working contextを削除し、Web UIの「会話を消す」はBrowser側の一時状態も削除する。API利用者は履歴なしで
従来どおり呼び出せる。

server-side stateが空の場合だけ、完成したuser / assistant pairを`source: client_history_hint`として
`conversation_context`へ投影する。client hintは表示上の継続材料であり、`last_llm_action`、Observation、active entity、
認可、Evidenceへ昇格させない。server-side `source: server_turn`が存在する場合は常にそちらを優先する。

Chat transport errorは公開detail文字列をrouting条件にせず、`X-Jarvis-Error-Code`の安定codeでChannel表示へ変換する。
現在は`chat_not_configured`を設定変更が必要な非retry error、`chat_turn_failed`を一時的に再試行可能なturn errorとして
区別する。error codeは回答やProvider選択ではなく、transport上の機械状態だけを表す。

process-local storeは1 sessionあたり最大5 turnに加え、無操作24時間のTTL、最大256 session、LRU evictionを持つ。
これはretention / resource境界であり、発話内容、topic、重要度、家族関係を評価しない。将来の永続Conversation
Repositoryでは認証済みprincipal、visibility、retention policyを正本とし、このprocess-local制限を認可の代替にしない。
