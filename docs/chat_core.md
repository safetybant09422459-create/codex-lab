...
2. Entity Resolution
   ...
   - v0.2では候補型と境界を定義し、Travel Search Index v0.1でTrip検索を最初の縦切りとして追加した。
3. Plan / Execute
...
## Travel Search Index v0.1

`backend/travel_search_index.py`は、Travel Skill Adapter側のTrip検索能力である。Chat Coreは
Travel固有語彙、Trip document、score規則、query expansionを持たず、共通形式の
`EntityCandidate`だけを扱う。

IndexはRuntime経由の`get_trips`結果から、`title`、`prefectures`、`memo`、
`start_date` / `end_date`、`outing_type`を検索documentとして組み立てる。NFKC、大小文字、
空白、依頼表現を正規化し、title完全一致、title部分一致、都道府県、memo、日付、
outing typeの順で基礎scoreを付ける。scoreは`0.0`から`1.0`に制限し、同点時は正規化titleと
Trip IDで安定順序にする。

query expansionはTravel側の小さな補助関数に閉じ、`神戸 -> 兵庫, 須磨`や
`シーワールド -> 須磨シーワールド, 水族館`など、少数の明確な関連語だけを追加する。
expansion一致は直接一致より低いscoreにし、辞書だけで検索結果を決めない。辞書を外部化する、
またはranking実装をBM25、SQLite FTS、Embeddingへ置き換える場合も、`search(query, trips)`から
`EntityCandidate`を返す境界とChat連携は維持できる。

名前付きTrip要求では、Chatはまず従来通りRuntimeで`get_trips`を取得し、その結果だけをIndexへ
渡す。候補が1件なら候補の実在するTrip IDでRuntimeの`get_trip`へ進み、複数件なら既存の
