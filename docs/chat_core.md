...
3. Plan / Execute
   ...
   - OrchestratorがPlanを受け取り、現在のbounded loop、Chat Tool Policy、Entity Resolver、Runtime実行を調整する。
...
`ComposeRequest`はOrchestratorが確定した事実だけをComposerへ渡す。現在はoutcome、Plan、
...
公開レスポンスの`action`、`reply`、`tool_id`、`arguments`、`result`、`candidates`、
...
3. PlanからRuntime step列を作る専用Plan Executorを抽出する。
4. Photo連携と`PhotoResponseComposer`をTravel / Photo双方のAdapter境界を通して追加する。
5. 更新ToolをRuntime確認付きPending Actionへ接続し、Composerのpending responseを実際の確認導線へ拡張する。
