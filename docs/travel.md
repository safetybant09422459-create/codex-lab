...
Checklist、Packing、Budget、SouvenirはTravelの主役ではない。扱う場合も補助機能であり、現時点の核はTrip / Outing、Candidate Spot、Spot、Move、Event、Memo、Cover Image、Photo Link、Memoryである。
...
* 訪問候補、採用済み地点、訪問済み地点、行かなかった地点をSpotとして管理する
* 時間制約のある移動予定や移動実績をMoveとして管理する
* Google Placesや写真に紐づかない出来事をEventとしてTimelineに置けるようにする
* Spot、Move、Event、MemoをTimeline Itemとして表示できるようにする
...
重要なのは、これはWeb UI向けの表示原則であり、MCP Tool、Chat、APIに同じ見た目を強制しないことである。一方で、Tool/APIが返すデータには、UIがこの体験を再現できるだけの`title`、`display_title`、時刻、`cover_image`、`item_type`、`participants`、`memo`、`linked_photos`などを含められるようにする。
...
Travel UIにおけるTimeline Itemは、単なるPlace CardではなくExperience Cardとして表示されることがある。これまでSpotと呼んでいたものも、実際には「朝散歩」「オルカショーでずぶ濡れ」「初めての海」のような体験タイトルを持つ。

そのため、Spot / Timeline Itemは「場所」だけでなく「体験」を表現できる必要がある。たとえば`place_name = マリンワールド`、`display_title = オルカショーでずぶ濡れ`のように、物理的な場所名とUI向け体験タイトルを分けられる設計にする。

Timeline Itemの種類:

* `place_spot`: Google Places参照を持つ、または場所として扱うSpot
* `move`: 新幹線、飛行機、予約済み交通など時間制約のある移動
* `event`: 出発、到着、チェックイン、家族内の節目など、場所Assetや写真がなくても置きたい出来事
* `memo`: Timeline上で見せたい短いメモ

Moveは主に時間制約がある移動を書く。座席番号や予約番号などの詳細は、ReservationまたはMoveのMemoに残す。

TimelineはTravelの状態から生成されるViewであり、UI専用ロジックではない。Jarvis CoreやMCP Toolからも、同じTimeline Itemとして取得できるようにする。
...
写真、メモ、時刻、体験タイトルが揃っている場合、Spot / Timeline Item自体がMemoryとして機能することがある。別EntityとしてのMemoryを急いで作る必要はなく、共有範囲、複数写真、独立した要約などが必要になった段階でMemory Entityを使えばよい。

SpotやMoveの滞在時間から、その時撮った写真候補をPhoto Skillへ問い合わせる。MemoryはTrip全体、Spot、Move、Event、Participantに紐づけられる。
