...
> Memoryを長期知識に限定し、現在の真実やObservationと分離する境界は
...
Memoryは会話を越えて保持する長期知識であり、現在の真実ではない。Conversation Stateは今回の会話、
Observationはこの会話で取得した証拠、Entity Contextは今回参照可能なEntity候補、Domain Providerは現在の
Source of Truthへの能力境界を担う。現在状態が必要ならDomain Providerから取得する。

LLMはMemoryを読むか、採用するか、Providerを呼ぶか、ObservationをMemory候補にするかを判断する。Python /
Memory Providerは選択済みOperationの保存、更新、削除、visibility、retention、redactionとRuntime safetyだけを
担う。重要度、推薦、意味的な検索順位、要約、回答生成、Provider / Operation選択、keyword / topic判定を持たない。
