...
> 失敗・訂正・改善シグナルをLong-term Contextから分離し、Jarvis改善専用のLearning Logとして扱う境界は
> [Jarvis Self-Improvement Principle](decisions/2026-07-jarvis-self-improvement-principle.md)を参照する。
...
失敗・訂正・改善シグナルは通常のContext Assemblyへ戻さず、別系統のLearning Logへ記録する。Learning Logは
Jarvis改善レビュー時だけLLMへ渡し、通常回答の根拠やユーザー理解には使わない。

```text
Conversation / Runtime / Observation / User correction
                    ↓
             Learning Log
                    ↓
        LLM improvement review
                    ↓
...
Long-term Context -> permitted context candidates within token budget
Activation RAG -> optional, unverified candidates only
```

Conversation Stateは会話履歴、Observationはこの会話で取得した事実、Long-term Contextは未来の推論を変える
長期間有効な文脈を扱う。Long-term ContextはRecordでもProviderのSource of Truthでもない。Observationは
...
| Jarvis Core | Skill、Tool、Runtime、Long-term Context、AI Agentをつなぐ中核 | 判断、調停、権限、Tool実行境界への接続 |
...
Jarvis Coreは、Jarvis全体の中核として、ユーザー、権限、Long-term Context、Tool、Runtime、AI Agentをつなぐ調停層である。

Coreは特定の画面、特定のSkill実装、特定のAI Provider、特定の外部APIに依存しない。

Coreが担当すること:

* ユーザー要求を、AI Agent、Tool、Long-term Context、Runtimeへ渡すための共通入口を持つ
* どのUser / Role / Contextで処理しているかを保持する
* Skill / Tool Registryを参照し、利用可能な能力を把握する
* Runtimeを通してTool実行、入力検証、権限確認、確認要求、監査へ進める
* visibility、permission、token budget内のLong-term Context候補をLLM入力へ組み立てる
...
* Long-term Contextの保存形式やretrievalアルゴリズムの詳細
...
### Long-term Contextとの境界

Long-term Contextは、呼び方、長期的な好み、判断基準、家族との関係、継続プロジェクト、設計思想など、
未来の推論を変える長期間有効な文脈である。Record、Observation、Conversation State、Provider結果、
Domain ProviderのSource of Truthではない。現在状態が必要ならDomain Providerから取得する。

Providerは出来事、記録、状態、操作対象という「What happened」を管理する。Long-term Contextは
「Why future reasoning should change」をLLMへ伝える。Pythonはvisibility、permission、token budgetと
候補retrievalまでを担い、候補の採用、組み合わせ、推論への反映はLLMが判断する。

Long-term Contextは全件をLLMへ渡さない。一部を将来Providerへ構造化する場合も、件数や保存都合ではなく、
新しい検索、操作、能力が生まれることを条件とする。詳細は
[Long-term Context Principle](decisions/2026-07-long-term-context-principle.md)を参照する。

### Learning Logとの境界

Learning Logは、失敗、ユーザー訂正、頓珍漢な回答、Provider / Operation選択ミス、Runtime失敗、Context不足、
Provider責務の曖昧さを、Jarvis自身の改善材料として保持する別系統の記録である。Long-term Contextのような
ユーザー理解ではなく、通常回答の根拠にも使わない。作業ログそのものはretention policyに従って削除できるが、
必要最小限にredactionした失敗・訂正・改善シグナルはLearning Logとして保持できる。Recordは各Providerの
Source of Truthとして、この棚卸しによる削除対象にしない。

LLMはLearning Logを定期または条件付きでレビューし、Provider Responsibility、Capability / Operation説明、
Context Assembly、Long-term Context retrieval、新Provider候補の改善提案を作れる。Learning Logへの記録、要約、
提案作成、docsやreview対象としての提示までは自動化できる。ただしProvider生成、DB作成、API / Tool追加、
コード変更は人間が判断する。

Pythonは保存、分離、visibility、permission、retention、redaction、token budget、決定的な構造化、Runtime記録を
担う。失敗の意味判定、改善提案生成、Provider化判断、ユーザー理解への変換、通常回答への利用判断は担わない。
棚卸しは毎日必須とせず、年単位、サイズ超過、明確な改善候補の増加時に行える。たとえばLong-term Contextが
100KBを超えたことは棚卸しを始める決定的な閾値候補になり得るが、削除・要約対象の意味判断はLLMまたは人間が行う。
...
Travelは最初のProvider / PoCである。Photo、Calendarは将来Providerとし、Homeは現実世界へ
...
* AI AgentへUser Context、Long-term Context候補、利用可能Tool、現在の制約を渡す
...
| --- | --- | --- | --- |
| Long-term Context | 未来の推論を変える長期文脈の候補を提供する | Coreがbudget内でLLM入力へ組み立てる | visibility、permission、最小開示が必要 |
...
| Learning Log / Reflection | 失敗、訂正、改善シグナルをユーザー理解と分離して保持・レビューする | 改善時だけLLMへ渡し、設計改善候補を作る | 通常回答やLong-term Contextへ混ぜず、自動実装しない |
| Agent | 意図理解、Tool選択、説明、提案を行う | Coreが制約付きで呼び出す判断層 | Tool直接実行を禁止する |
| Conversation | チャットや音声の会話状態を管理する | Coreへの入口のひとつ | 会話ログとLong-term Contextを混同しない |
| Personality | Jarvisらしい判断傾向、言い方、優先順位を形成する | Long-term Context、Vision、Decision、Lessonから構成される | 口調だけに閉じない |
...
| Event Bus | Tool実行、通知、監査などのイベントを流す | Core内外の疎結合化に使う | 更新系イベントの重複実行に注意 |
| Access Control | User、Role、Visibility、Resourceごとの認可を扱う | Core / Runtime / Long-term Context / Skillが参照する | 家族プライバシーの中心になる |
...
Jarvisの人格と継続性を支え、未来の推論を変える。単なる会話ログや記録の保存層ではない。
...
* Lessons Learned
* User Preferences
* Long-term Preferences and Relationships

失敗や訂正はLong-term ContextではなくLearning Logで扱う。
