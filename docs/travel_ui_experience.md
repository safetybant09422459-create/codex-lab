...
これは、すべてのカードを別の保存モデルにするという意味ではない。Spot、Event、Move、Timeline ItemのUI向け表現が、Experience Cardとして振る舞える必要があるという意味である。
...
データモデル上の名称を今すぐSpotからExperienceへ一括変更する必要はない。重要なのは、Spot / Timeline Itemが場所だけでなく体験を表現できることである。
...
* `place_name`: 物理的な場所名
* `item_type`: `place_spot`, `experience`, `move`, `event`, `memo`
...
* Memoryは、最初から必ず別Entityにしなくてもよい。十分な情報を持つSpot / Timeline ItemがMemoryとして機能する場合がある。
