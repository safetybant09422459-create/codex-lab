...
[Observation Guardrail](decisions/2026-07-observation-guardrail.md)を参照する。Observationは観測事実であり、
解釈、次Action、回答、Provider / Operation選択、UI表示判断ではない。
[Observation Reference Principle](decisions/2026-07-observation-reference-principle.md)に従い、Observationは取得時点の
会話証拠であって現在値のキャッシュではない。現在の真実はProviderから取得し、過去のObservationだけで十分か、
Providerを再実行するかはLLM / Jarvis Coreが判断する。

read-only Provider Operationの実行結果は、Agent Host境界でObservation Envelopeへ変換してLLMへ渡す。
Envelopeは`provider_id`、`operation_id`、`status`、必要な範囲で認可・sanitizationした`raw_result`、Providerが結果から決定的に抽出した
...
PythonはObservationの保存、visibility、size制限、redactionを担えるが、stable / volatile分類やfreshness閾値から
Provider再取得を判断してはならない。`freshness` / `observed_at`はLLMが十分性を判断する材料であり、cache TTLではない。
