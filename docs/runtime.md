...
Travel Runtime Read v0.1では、Travel Skillの読み取りToolを `local_travel_read` としてRuntime safety layer経由で実行する。
...
* `WeatherExecutor`
* `TravelExecutor`
* Travel Runtime Read v0.1

Not Yet Implemented:

* Confirmation UI
* 外部APIを使うReal Tool Execution
* DB-backed Travel Runtime
* Travel write tools

`execute_stub` は確認が不要、または確認済みで、入力検証に成功した場合、`ExecutorRegistry` から取得したexecutorを呼ぶ。Weather Skillは `WeatherExecutor` を呼び、`execution_mode: "local_weather_stub"` と副作用のないローカル結果を返す。Travel Skillの実装済み読み取りToolは `TravelExecutor` を呼び、`execution_mode: "local_travel_read"` とローカル読み取り結果を返す。その他の未実装Toolは `StubExecutor` を呼び、`execution_mode: "stub"` とstub結果を返す。
...
Travel Runtime Read v0.1は `backend/travel_executor.py` に実装している。

現在のTravel Runtime Read v0.1:

* Travel Skill の読み取りToolのみ
* `local_travel_read`
* `TravelExecutor`: `backend/travel_executor.py`
* `TravelRepository`: `backend/travel_repository.py`
* `TravelSource`: `backend/travel_sources.py`
* 外部Travel DBなし
* 副作用なし
* read / low risk
* Runtime safety layer 経由で実行
...
`travel.get_spots` (`get_spots`) はTool定義のみ存在し、Travel Runtime Read v0.1の実行対象外である。
