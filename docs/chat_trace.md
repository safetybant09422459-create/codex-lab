...
保存する環境情報はproject root、Git SHA / branch / dirty、サービス開始時刻、Python / OpenAI SDK版、Jarvis app title、設定モデルと推論設定である。API key、Authorization、Cookie、Developer Token、環境変数一覧、`.env`は保存しない。Git情報は起動時に対象project rootを明示した`git -C <root>`で一度だけ取得する。dirtyはclean時`false`、dirty時`true`、取得失敗時`unknown`とし、subprocessのstderrはTraceへ保存しない。
...
Responses APIの`usage`から、input、cached input、output、reasoning、total tokenを存在する場合だけ取得する。各LLM callの内訳とTurn合計を分けて表示し、呼び出し数が増えても`llm_call_N`として追加できる形にする。欠落値は推測せずnullとし、`LLM_USAGE_MISSING`をinfo候補として残す。料金推定は行わない。
...
個別fieldと配列を制限し、切り詰め時は`_truncated` / `_original_bytes` / `_preview`または`_items_truncated` / countを残す。相談用Bundleは40KBを上限に、Context、Operation Catalog、LLM request / response、Runtime、Observationを診断用の専用summaryへ投影し、TurnContext、LLMInputPayload、Tool / JSON Schema、raw result全体を含めない。完全Bundleは200KBを上限に、それらのsanitized詳細を維持する。

## Anomaly Flags

例外categoryとStageの観測結果だけを使う決定的ルールで、LLM request / timeout / refusal / incomplete / usage欠落、Action validation、Observation後のOperation、同一Operation再選択、Runtime / Permission / Confirmation、Observation欠落・失敗、Final LLM / Action / Answer、Turn未完了を候補表示する。性能警告はTurn totalが20,000 tokens以上の`HIGH_TOKEN_USAGE`、単一LLM callが5,000ms以上の`SLOW_LLM_CALL`、Turnが10,000ms以上の`SLOW_TURN`で、いずれもwarningとする。Token Usage未取得時は`HIGH_TOKEN_USAGE`を出さない。原因を断定せず、自然言語の意図解釈やLLMによる原因推定は行わない。
