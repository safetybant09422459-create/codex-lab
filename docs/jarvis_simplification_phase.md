...
* `TravelPlanner`と`chat_router`は意味判断をLLMへ委譲しているが、独立した複数LLM段として残る。
  vNextでは単一LLM Agent Loop stateへ統合する。
* `TravelEntityResolver`の公開互換クラスは残るが、常に`needs_context`を返し意味解決しない。
  呼び出し元移行後に削除する。
* `TravelSearchIndex`とActivation RAG lexical indexは候補想起用として残る。候補をEvidence、Entity確定、
  権限、Tool実行許可へ昇格させてはならない。
* Composerには構造的なstatus文言が残る。自然言語質問の内容で文言を分岐しない。
