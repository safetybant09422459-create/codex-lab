...
* Spot、Move、Event、Reservation、Memo、Memory、Checklist、Packing、Budgetは原則Tripにぶら下げる
* Spot / Timeline Itemは物理的な場所だけでなく、家族のおでかけにおける体験を表現できるようにする
* Spotに強く紐づく情報はSpotにもぶら下げられるようにする
* 同じ時間帯の複数候補をCandidate Spotとして保持できるようにする
* TimelineはSpot + MoveだけでなくEventやTimeline用Memoも含めて生成できるようにする
* WishlistはTrip作成前の独立データとして扱える
* 旅行文脈に閉じる情報はTravelに置く
* 汎用化が必要になったら、将来Skill分離候補にする

Packing、Budget、Checklist、Souvenirは補助機能である。現時点の核は、Trip / Outing、Candidate Spot、Spot、Move、Event、Memo、Cover Image、Photo Link、Memoryである。
...
Tripは、タイトル、日程、都道府県、参加者、Candidate Spot、Spot、Move、Event、Memo、Cover Image、Photo Link、Memoryを束ねる中心である。
...
旅行やおでかけで訪問する候補地、採用済み地点、実際に訪問した地点を管理する。

SpotはGoogle PlacesのPlaceそのものではなく、家族のおでかけにおける訪問地点である。

さらにTravel UIでは、Spotは単なる場所ではなく体験を表すことが多い。Timelineカード上では「マリンワールド」という場所名よりも、「オルカショーでずぶ濡れ」「オルカ見ながらビュッフェ」「初めての海」のような体験タイトルが主表示になる場合がある。

そのため、SpotはPlace CardではなくExperience Cardとして表示できる情報を持つ。データモデル上の名称を今すぐすべてExperienceへ変更する必要はないが、Spotは場所と体験の両方を表現できる前提にする。

計画中は、同じ時間帯に複数のCandidate Spotが並ぶことがある。これは未確定計画の重要な状態であり、あとで採用、削除、時間変更、別時間帯への移動を行う。
...
* `location`
* `provider_place_ref`
* `item_type`: `place_spot`, `experience`
* `status`: `candidate`, `planned`, `confirmed`, `visited`, `skipped`, `cancelled`, `unplanned`
* `candidate_group_id`
* `planned_start_at`
* `planned_end_at`
...
Spot固有のMemo、Memory、Review、Reservation、Checklistを追加でぶら下げられる。

### planned / actual

Spotはplanned/actualとstatusが特に重要である。
...
Moveは主に時間制約がある移動を書く。新幹線、飛行機、予約済み交通などが中心であり、座席番号や予約番号はReservationまたはMoveのMemoに残す。
...
Google Placesにも写真にも紐づかないが、Timeline上に置きたい出来事を管理する。
...
* `title`
* `description`
* `event_type`: `departure`, `arrival`, `check_in`, `meetup`, `rest`, `family_moment`, `other`
* `planned_at`
* `actual_at`
* `status`: `planned`, `happened`, `skipped`
...
必要に応じてMemoやMemoryから参照できる。

### planned / actual

Eventは予定としてTimelineに置く場合と、旅行後に実績として追加する場合がある。

### 分類

* read/write/mixed: `mixed`
* risk level: `low`から`medium`
* confirmation: 削除、共有時に必要
* audit: 共有やprivate情報を含む場合は必要

### 他Skillとの境界

EventはCalendar全体の予定ではない。

TravelのEventはTrip / OutingのTimeline上の出来事であり、Google PlacesやPhoto Assetがなくても家族の記憶として残すために使う。
...
Trip / Outing、Spot、Move、Event、MemoryとPhoto SkillのAsset候補を接続する。
...
* `photo_link_id`
* `trip_id`
* `owner_type`: `trip`, `spot`, `move`, `event`, `memory`
* `owner_id`
* `asset_ref`
...
写真、メモ、時刻、体験タイトルが揃っているSpot / Timeline Itemは、それ自体がMemoryとして機能する場合がある。別EntityとしてのMemoryは、独立した共有範囲、複数写真、要約、ハイライト選択が必要になったときに使えばよい。
...
必要に応じてSpot、Move、Event、Participantにも紐づける。

### planned / actual

Memoryは実績側のデータである。

SpotやMoveの滞在時間、Event時刻、Photo Linkを使い、写真と思い出メモを組み合わせる。
