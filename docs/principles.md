...
Provider Operation契約、Capability説明、Long-term Context、LLM promptを改善して上げる。Pythonへ自然言語keyword分岐、
...
Long-term Context Changes Future Reasoning
...
将来のLLMの判断へ継続的に影響する文脈を扱う。Record、Observation、Conversation State、Provider結果、
Domain ProviderのSource of Truthではない。

Long-term Contextは無制限にLLMへ渡さない。Pythonはvisibility、permission、token budgetと候補retrievalを
決定的に適用し、候補のうち何を推論に利用するかはLLMが判断する。Pythonは意味的な採否、重要度、推薦、
