...
ChatはToolの実行入口の1つである。Basic ChatはToolなしで完結できる。Capabilityが必要な場合もmodelへ
自由な実行権限を与えず、server-side allowlist、構造化引数、Runtime、write確認、上限設定、Auditを
必須とする。Chat回答はRepositoryから再取得したEvidenceを根拠にし、Activation RAG候補、確認できない
写真内容、存在しないIDを事実として扱わない。
