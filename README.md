...
Developer APIはデフォルト無効である。Codex、project / audit / logs、git changes / diff / commit-push、service操作を
利用する場合だけ、server側で`JARVIS_ENABLE_DEVELOPER_API=true`と十分に長いランダムな
`JARVIS_DEVELOPER_TOKEN`を設定し、`Authorization: Bearer` headerで送る。query parameterは利用できない。
tokenをログ、Audit、response、frontendへ記録しない。詳細と保護endpointは
[Jarvis Trust Boundary v0.1](docs/trust_boundary.md)を参照する。

これは同一process内の緊急封じ込めであり、次段階ではConsumer Plane / Developer Planeを別process、port、Unix userへ
分離する。
